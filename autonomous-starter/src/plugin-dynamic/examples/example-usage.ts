...
import { pluginDynamic } from "../index";

/**
 * Example: Creating a Weather Plugin
 */
async function createWeatherPlugin(agent: Agent) {
 ...
 */
async function createTodoPlugin(agent: Agent) {
  const description = `
 ...
 */
async function monitorPluginCreation(agent: Agent) {
  // Check status
