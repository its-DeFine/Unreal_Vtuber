...
| Payments backend (3.141.111.200) | 9090                          | –                |
   ...
   **Example (UFW)**
   ...
   sudo ufw reload
   ```
5. Launch the Pixel Streaming stack (includes the health monitor service). Double-check `.env` still contains `VTUBER_ALLOWED_ADDRESSES=3.150.172.153` before running compose so the runner trusts the forwarder IP:
   ```bash
