...
On EC2, it will also try to apply the required inbound security group rules (best-effort) if `aws` CLI + permissions are available; disable with `--no-apply-firewall`.
