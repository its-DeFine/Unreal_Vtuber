...
def ops_wormhole_receive(
    ...
    full_cmd = (
        ...
        f"wget -q -O /usr/local/bin/wormhole-william {shlex.quote(_WORMHOLE_WILLIAM_URL)} "
        ...
        f"timeout {q_timeout} /usr/local/bin/wormhole-william receive {q_code} > /tmp/image.tar.zst "
        f"2>> {q_log}; "
        f"echo \"[wormhole] receive exit=$?\" >> {q_log} 2>&1; "
        f"echo '[load] Loading image via docker load...' >> {q_log} 2>&1; "
        f"zstd -d /tmp/image.tar.zst | docker load >> {q_log} 2>&1; "
