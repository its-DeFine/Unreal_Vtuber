...
4. **Recorder Manager**
   ...
   - Runs a lightweight `aiohttp` service that listens for session notifications from the script runner.
   - Launches `PixelStreamingRecorder` when a payload begins, stores the capture under `/captures`, and optionally uploads it to the storage service once the session ends.
   - Exposes environment variables to tune bitrate, container format, preferred spatial layer, upload behaviour, etc.
...
- **Recorder policies live in one place**: The script runner only emits start/stop webhooks; the `recorder-manager` container owns bitrate, upload, and storage configuration. Update the `RECORDER_*` environment variables instead of editing multiple scripts.
