...
3. **TURN**
   ...
   - TURN is configured with `--use-auth-secret` and `--static-auth-secret`, so client tokens are HMAC-SHA1(username, secret). Generate tokens with `TURN_SECRET=... node scripts/gen_turn_token.js` and hand username/password/realm to the client.
