# Pixel Streaming Architecture & Operations

This document summarizes how the Unreal VTuber repository packages the Pixel Streaming stack, the lessons learned while containerizing it, and the main operational considerations when deploying on EC2 or on local hosts.

## High-Level Layout

```
+----------------------------------+        +----------------------------------+
| vtuber-unreal-game (container)   |        | vtuber-unreal-signaling (container)
|----------------------------------|        |----------------------------------|
| Image: ghcr.io/.../embody-ue-ps  |  --->  | Image: embody-signaling:latest
| Runs: Embody build + Pixel       |  WebRTC| Runs: Epic start.sh (Wilbur) with
| Streaming streamer, Xvfb         | stream | bundled node/npm dependencies
| ulimit: 1040 (nofile)            |        | Exposes 8080/8888/8889
+----------------------------------+        +----------------------------------+
                  |                                    |
                  |                                    v
                  |                        +-------------------------+
|                        | turn-server (coturn)    |
|                        | (optional) UDP/TCP 3478 + 49160-49200
                  |                        | Credentials from .env.turn
                  |                        +-------------------------+
                  v
        Local/remote recorder invocations (manual)
```

All services share the external Docker network `vtuber_network`, so the streamer connects to the signaling container using the internal DNS name `unreal-signaling`.

## Container Build Pipeline

1. **Game Image (`embody-ue-ps:latest`)**
   - Dockerfile: `docker/pixel-streaming/game/Dockerfile`.
   - The CI workflow copies the packaged Pixel Streaming build into `linux-pixel-streaming/` before running `docker build`.
   - The container entrypoint (`/usr/local/bin/start-embody.sh`) launches Xvfb (optional), applies `ulimit`, then runs `./Embody.sh` with `-RenderOffScreen` and `-PixelStreamingURL=...`.
   - Note: packaged builds only emit useful runtime logs when launched with `-log` (aka `-Log`). Our entrypoint includes `-Log` so `docker logs vtuber-unreal-game` is actionable.
   - Note: use `COPY --chown=...` and avoid `chown -R` or broad `chmod` steps over the whole build tree; those create large duplicate layers and bloat the image.

2. **Signaling Image (`embody-signaling:latest`)**
   - Dockerfile: `docker/embody-app/Dockerfile`.
   - Runs Pixel Streaming SignallingWebServer (Wilbur) and patches it to support Cirrus-style Matchmaker registration plus optional join-token gating.
   - Does not ship the Pixel Streaming UI bundle; the edge/gateway serves the UI and proxies websocket traffic to orchestrators. Health endpoint: `GET /healthz`.
   - The published image is an “app bundle” that also contains the Python runner + recorder-control processes under `supervisord`. The default compose stack still runs those as separate services because the Unreal command port binds to `127.0.0.1` inside the game container and recordings need a mounted `/recordings` volume (configured via `VTUBER_RECORDINGS_DIR`, defaults to `/recordings`).

3. **TURN**
   - Uses the `embody-turn-server:latest` image. Credentials and ports are sourced from `.env.turn` (generated by `./scripts/generate_turn_credentials.sh`).

## Lessons Learned

- **Bundled Node/npm must be present**: The Pixel Streaming scripts expect their own Node runtime under `platform_scripts/bash/node/bin`. Missing this caused `Cannot find module 'express'`. Packaging must include both `NODE_VERSION` and the Node tree.
- **`setup.sh` needs internet at build time**: Running it during the Docker build ensures Wilbur is ready. If run at runtime, the container would require outbound access, which complicates deployments.
- **CA certificates are required**: `setup.sh` and `start.sh` use `curl https://api.ipify.org` and other TLS endpoints. Installing `ca-certificates` inside the container prevents `curl: (77)` errors.
- **File descriptor limits**: The game crashes or loses websocket connections if `nofile` is too low. We set `HOST_ULIMIT_NOFILE=1040` and pass `ULIMIT_NOFILE=1040` into the game container so `/usr/local/bin/start-embody.sh` applies the correct limit.
- **TURN reachability is optional**: The recorder runs on the orchestrator and connects over the Docker network, so no additional ports need to be exposed publicly. Enable TURN only when remote viewers must reach the stream across restrictive networks.
- **PixelStreaming2 codec control can be `config=Game`**: UE5.4+/UE5.5+ PixelStreaming2 can ignore legacy codec CLI flags (for example `-PixelStreamingDisableVP9`) and silently default to VP9. Some packaged builds load the settings via `[/Script/PixelStreaming2Settings.PixelStreaming2PluginSettings]` (`Codec`, `LatencyMode`, `KeyframeInterval`), so we ship both `pixel-streaming/config/Engine.ini` and `pixel-streaming/config/Game.ini` and mount them into `Saved/Config/...` for maximum compatibility.
- **Compose vs. manual scripts**: Running Epic’s `start.sh` on the host helped identify missing dependencies, but ultimately the goal is a self-contained container. The current Dockerfile mirrors those host steps so compose deployments are turnkey.
- **Recorder runs on demand**: The headless recorder is now a manual step (either on the orchestrator or from a whitelisted workstation). Use the CLI flags/`RECORDER_*` environment variables to tune bitrate/quality per capture.

## Operational Checklist

1. `git pull --rebase origin feature/vtuber-elevenlabs`
2. `./scripts/generate_turn_credentials.sh`
3. (Optional) `docker/aws-pixel-streaming/package-embody.sh /path/to/Embody/Linux`
4. `docker compose -f docker-compose.unreal.yml build unreal-signaling`
5. (Ensure `.env` includes `VTUBER_ALLOWED_ADDRESSES=<edge-ip>` (example: `3.150.172.153`), then) `docker compose -f docker-compose.unreal.yml up -d unreal-signaling unreal-game turn-server`
6. Verify logs and ulimit: `docker logs vtuber-unreal-signaling --tail 20`, `docker exec vtuber-unreal-game bash -lc 'ulimit -n'`
7. Connect via browser on the orchestrator (`http://127.0.0.1:8080`, or tunnel the port over SSH if you need to view it remotely)

Keep `.env.turn` secure—the credentials are shared between coturn and Wilbur. Regenerate the file whenever redeploying to rotate secrets.

Run the payments backend from the standalone payments repository whenever
payouts or orchestrator monitoring are required.
