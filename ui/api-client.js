// API Client for Autonomy UI - Backend Integration via Bottle Proxy
class APIClient {
    constructor() {
        this.baseURLs = {
    ...
    async getGPUStatus() {
        try {
            return await this.makeRequest(`${this.baseURLs.autogen}/gpu-status`);
        } catch (error) {
    ...
    async exportSemanticMap() {
        try {
            return await this.makeRequest(`${this.baseURLs.semantic}/export?format=d3js`);
        } catch (error) {
